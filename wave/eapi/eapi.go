...
/*func getLog(c context.Context) error {
        logLevel := c.GlobalString("log")
        // set variables to default level (error) 
        var traceHandler, infoHandler, warningHandler, errorHandler io.Writer
        //logic for logging
        if logLevel == "trace" || logLevel == "Trace" {
                traceHandler, infoHandler, warningHandler, errorHandler = os.Stdout, os.Stdout, os.Stdout, os.Stderr
        } else if logLevel == "info" || logLevel == "Info" {
                traceHandler, infoHandler, warningHandler, errorHandler = ioutil.Discard, os.Stdout, os.Stdout, os.Stderr
        } else if logLevel == "warning" || logLevel == "Warning" {
                traceHandler, infoHandler, warningHandler, errorHandler = ioutil.Discard, ioutil.Discard, os.Stdout, os.Stderr
        } else {
                traceHandler, infoHandler, warningHandler, errorHandler = ioutil.Discard, ioutil.Discard, ioutil.Discard, os.Stderr
        }
...
func (e *EAPI) CreateEntity(ctx context.Context, p *pb.CreateEntityParams) (*pb.CreateEntityResponse, error) {
	//The function starts by error checking the entered storage server IP and Port (RevocationLocation), or set RevocationLocation to the default found in the config file 
	// default config file located in /etc/wave/wave.toml 
/*        err := getLog(ctx)
        if err != nil {
                fmt.Printf("error: %v\n", err)
                os.Exit(1)
	...
	//iapi/entity.go, expected return PublicDER and SecretDER
